...
6. Contract documents tied to a deal: extend the existing `/documents` upload/reassign flow so a document can link to a `dealId` in addition to (or instead of) a `clientId`. **Not built yet.**
